...
fn main() {
  ...
  let sig = dev.create_signal::<f64, 1>("test_sin", mpr_dir::MPR_DIR_OUT);
  let debug_sig = dev.create_signal::<f64, 1>("debug_msg", mpr_dir::MPR_DIR_IN);
  loop {
      ...
      let msgs = debug_sig.get_value_single(0);
      if msgs.is_some() {
